...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query("select u from Event u where u.eventType = 'CampVisited' and u.campId = :campId and u.eventDate >= :lowDateRange and u.eventDate <= :highDateRange")
    ...
    @Query("select u from Event u where u.eventDate >= :lowDateRange and u.eventDate <= :highDateRange order by u.eventDate")
    List<Event> getAllEvents(@Param("lowDateRange") Timestamp lowDateRange, @Param("highDateRange") Timestamp highDateRange);

    @Query("select u from Event u where ( u.eventType = 'AssignLeadCampMgr' or  u.eventType = 'AssignCampMgrOne' or  u.eventType = 'AssignCampMgrTwo')  and u.campId = :campId and u.eventDate >= :lowDateRange and u.eventDate <= :highDateRange")
